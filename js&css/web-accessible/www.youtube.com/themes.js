...
ImprovedTube.myColors = function () {
	if (this.storage.theme === 'custom') {
		...
		if (primary_color) {
			if ((0.2126*primary_color[0] + 0.7152*primary_color[1] + 0.0722*primary_color[2]) / 255 < 0.4) {style.textContent += ' .improvedtube-player-button svg,.improvedtube-player-button svg path{fill:#fff!important} '}
